...
import { TrendingUp, TrendingDown, Minus, AlertTriangle, CheckCircle, Info, Wind, Car, Factory, Construction, Leaf, Sun, Cloud, Droplets, Thermometer, Target, BarChart3, Sparkles } from "lucide-react"
...
export default function AIForecast({ selectedWard }: AIForecastProps) {
  ...
  const metrics = [
    { id: "aqi", label: "AQI", color: "hsl(var(--primary))", icon: <Thermometer className="w-4 h-4" /> },
    { id: "pm25", label: "PM2.5", color: "hsl(var(--primary))", icon: <Wind className="w-4 h-4" /> },
    { id: "pm10", label: "PM10", color: "hsl(var(--primary))", icon: <Cloud className="w-4 h-4" /> },
    { id: "no2", label: "NO2", color: "hsl(var(--primary))", icon: <Factory className="w-4 h-4" /> },
    { id: "o3", label: "O3", color: "hsl(var(--primary))", icon: <Sun className="w-4 h-4" /> },
  ]
            ...
            {getSuggestions().map((suggestion, index) => (
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-6 h-6 text-primary">{suggestion.icon}</div>
      ...
      {!loading && forecastData.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="rounded-3xl border border-border/40 glass-effect p-6 text-center">
            ...
            <p className="text-3xl font-bold text-primary">{forecastData.length}</p>
